...
if os.path.exists("checkpoint_last_512_logits.pt"):
    checkpoint = torch.load("checkpoint_last_512_logits.pt", map_location=device)
    ...
    for e in range(epochs):
        ...
        if avg_val_iou > best_miou:
            ...
            torch.save({
                ...
                "best_val_loss": min_loss,
                "best_val_miou": best_miou,
            }, "checkpoint_best_miou_512_logits.pt")
...
#history = train(50, model, train_loader, val_loader, loss1, loss2, iou_metrics, optimizer)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("grafico_loss_512_logits.png", dpi=300, bbox_inches="tight")
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("grafico_miou_512_logits.png", dpi=300, bbox_inches="tight")
plt.show()
...
# =========================
if os.path.exists("checkpoint_best_miou_512_logits.pt"):
    checkpoint_test = torch.load("checkpoint_best_miou_512_logits.pt", map_location=device)
    model.load_state_dict(checkpoint_test["model_state_dict"])
    print("Caricato checkpoint_best_miou_512_logits.pt per il test finale")
else:
    print("checkpoint_best_miou_512_logits.pt non trovato, uso il modello corrente")
model.eval()
test_loss = 0.0
test_iou = 0.0
...
with torch.no_grad():
    for image, mask, file_name in test_loader:
        ...
        for b in range(current_batch_size):
            ...
            if is_positive:
                positive_shown += 1
                save_name = f"test_positive_example512_logits{positive_shown}.png"
            else:
                negative_shown += 1
                save_name = f"test_negative_example512_logits{negative_shown}.png"
